...
$sql="SELECT nombres, idclie, ruc, barrio, direccion, telefono FROM clientes WHERE nombres LIKE '%".$criterio."%' or barrio LIKE '%".$criterio."%' or ruc LIKE '%".$criterio."%'";
...
while($dt=pg_fetch_array($datos)){
	if ($contador > 0) {$resul.= ", ";}
	$des=trim($dt['nombres']).'|'.trim($dt['barrio']).'|'.trim($dt['ruc']).'|'.trim($dt['direccion']).'|'.trim($dt['telefono']);
